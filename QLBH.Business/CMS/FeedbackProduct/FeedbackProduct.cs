    ...
    public class FeedbackProduct : IFeedbackServices<Response_Feedback>
    {
        ...
        public async Task Delete(long id)
        {
            try
            {
                ...
                var productID = query.Select(item => item.ProductID);
                await UpdateEvaluate(productID.ToList());
            }
            catch (Exception ex)
            {
        ...
        public async Task UpdateEvaluate(List<long> values)
        {
            foreach (var id in values)
            {
                await UpdateEvaluate(id);
            }
        }
        ...
        public IEnumerable<Response_Feedback> GetAll(long accountId = 0, long productId = 0)
        {
